...
function [lik, latents] = twoAlpha_oneBeta(QbanditOrder, agencyChoiceVec, banditChoiceVec, outcome, agency, offer, leftBandit, rightBandit, x, priors)
...
for ii = 1:length(outcome)
    ...
    if agency(ii) == 1
        ...
        %update loglikelihood of choice
...
if (priors)
    lik = lik + log(betapdf(alphaChoice,1.1,1.1));
    lik = lik + log(betapdf(alphaComp,1.1,1.1));
