...
function [lik, latents] = fourAlpha_twoBeta_agencyBonus(QbanditOrder, agencyChoiceVec, banditChoiceVec, outcome, agency, offer, leftBandit, rightBandit, x, priors)
...
if (priors)
    lik = lik + log(betapdf(alphaChoicePos,1.1,1.1));
    lik = lik + log(betapdf(alphaChoiceNeg,1.1,1.1));
    lik = lik + log(betapdf(alphaCompPos,1.1,1.1));
    lik = lik + log(betapdf(alphaCompNeg,1.1,1.1));
